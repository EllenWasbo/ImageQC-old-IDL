...
function calculateMTF, submatrix, pix, center, typeMTF, backmatrix, cutLSF, cutW, cutF, sampFreq
  ...
  CASE typeMTF OF
    ...
    1: BEGIN; wire (3d)
      ...
      background1=MEAN(subMatrix[0,0,*])
      background2=MEAN(subMatrix[0,szM(1)-1,*])
      background3=MEAN(subMatrix[szM(0)-1,szM(1)-1,*])
      background4=MEAN(subMatrix[szM(0)-1,0,*]);mean of all pixels of all corners
      background=MEAN([background1,background2,background3,background4])
      ...
      ;yfit=gaussfit(newdists, pixValsInterp, A, NTERMS=4)
      ;bg=A[3]
      bg=MEAN([pixValsInterp[10:30],pixValsInterp[nn-30:nn-10]])
      pixValsInterp=pixValsInterp-bg
      pixVals=pixVals-bg
