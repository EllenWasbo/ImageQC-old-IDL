...
s=275
d2=137;pix 0,201; 55 for pix 0,5
A=[700.0,-40,0.7*10,2.0*10];[700.0,-40,0.7*10,2.0*10];[600.,0.,.7*10,2.*10];
radFraction=0.01; .01=bead, .6 brukt til disk forsok
outVal=100
inVal=9000;-1000
nImg=2
noiseFactor=20;200;50

;center=[s,s+5];[s+5,s+9,s+12,s+19]
center=[s,s]

matrix=FLTARR(s*2+1,s*2+1,nImg)+outVal

FOR i=0,nImg-1 DO BEGIN
  circle=getROIcircle([s*2+1,s*2+1], [center(i),center(i)], s*radFraction);arrSz, center, radius
  matrix[*,*,i]=matrix[*,*,i]+circle*(inVal-outVal)
ENDFOR
...
FOR i=0,nImg-1 DO matrixConvol[*,*,i]=SHIFT(FFT(FFT(matrix[*,*,i])*FFT(kernel),/INVERSE),s,s)*((s*2.+1)^2); *N^2

matrixScaledDown= CONGRID(matrixConvol, d2,d2,nImg,/Center,cubic=-0.5)

noiseMatrix=noiseFactor*randomn(seed,d2,d2,nImg)
