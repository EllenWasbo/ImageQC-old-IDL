...
pro ImageQC_event, ev
  ...
  IF N_ELEMENTS(uval) GT 0 AND SIZE(uval, /TNAME) EQ 'STRING' THEN BEGIN
    ...
    CASE uval OF
      ...
      'backupConfig':BEGIN
        adr=DIALOG_PICKFILE(TITLE='Backup config file', /WRITE, FILTER='*.dat', /FIX_FILTER, DIALOG_PARENT=evTop, /OVERWRITE_PROMPT, DEFAULT_EXTENSION='.dat', PATH='C:\')
        IF adr(0) NE '' THEN FILE_COPY, thisPath+'data\config.dat', adr
      END

      'restoreConfig':BEGIN

        adr=DIALOG_PICKFILE(TITLE='Locate config file to restore from', /READ, FILTER='*.dat', /FIX_FILTER, DIALOG_PARENT=evTop, PATH='C:\')
        IF adr(0) NE '' THEN BEGIN
          ...
          IF SIZE(newConfigS, /TNAME) EQ 'STRUCT' THEN BEGIN
            ...
            sv=DIALOG_MESSAGE('Config file restored.', /INFORMATION, DIALOG_PARENT=evTop)
      ...
      'openMulti':BEGIN
        ...
        ENDIF ELSE BEGIN
          QTtempArr=quickTemp.(sel-1)
          tempnames=TAG_NAMES(quickTemp)
      ...
      'openAuto': BEGIN
        ...
        ENDIF ELSE BEGIN
          ...
          FOR i=0, N_ELEMENTS(tempnames)-1 DO BEGIN
            ...
            IF adrTempTemp(0) NE '' THEN tempnames(i)=tempnames(i)+' ('+STRING(N_ELEMENTS(adrTempTemp),FORMAT='(i0)')+')'
          ENDFOR
          ...
          If res.Get THEN BEGIN
            ...
            IF adr NE '' THEN BEGIN
              ...
              IF adrTempTemp(0) NE '' THEN BEGIN
                statNames=STRARR(N_ELEMENTS(tempnames))
                loadAdr=STRARR(N_ELEMENTS(tempnames))
                FOR i=0, N_ELEMENTS(tempnames)-1 DO BEGIN
                  statNames(i)=loadTemp.(i).statName
                  loadAdr(i)=loadTemp.(i).path
                ENDFOR
                ...
                FOR n=0, nn-1 DO BEGIN
                  ...
                  IF dcm(n) NE 0 THEN BEGIN
                    ...
                    ;filename: statname_date_time_protocol
                    ...
                    IF acqdate NE '' AND acqtime NE '' THEN BEGIN 
                      If stationName NE '' THEN basename=stationName+'_' ELSE basename=''
                      ...
                      ENDELSE
                      ;ensure path ending with separator
                      IF STRMID(newAdr(0), 0, /REVERSE_OFFSET) NE PATH_SEP(SEARCH_PATH=newAdr(0)) THEN newAdr=newAdr(0)+PATH_SEP(SEARCH_PATH=newAdr(0))
  
                      WIDGET_CONTROL, lblProgress, SET_VALUE='Moving and renaming file '+STRING(n, FORMAT='(i0)')+' / '+STRING(nn, FORMAT='(i0)')
                      file_move, adrTempTemp(n), newAdr(0)+basename+'.dcm'
      ...
      IF proceed GT 0 THEN BEGIN
        CASE proceed of
          1: autoTempRun, loadTemp.(selTemp)
          2: BEGIN
            
            FOR i=0, N_ELEMENTS(tempnames)-1 DO BEGIN
              IF tempnames(i).substring(-1) EQ ')' THEN BEGIN
                autoTempRun, loadTemp.(i)
              ENDIF
            ENDFOR
    ...
    'filelist' : BEGIN
      IF tags(0) NE 'EMPTY' THEN BEGIN
        ...
        ;IF ev.clicks EQ 2 THEN BEGIN; need to click extremely fast
        ;this makes trouble when scrolling - also regarded as doubleclick when not intended
...
;        IF selDiff NE 0 THEN redrawImg, 0,1 & updateInfo
        ...
        IF sel(0) GT 0 THEN BEGIN; overwrite
          IF szQT EQ 'STRUCT' THEN BEGIN
            ...
            IF sv EQ 'Yes' THEN BEGIN
              quickTemp=replaceStructStruct(quickTemp, markedMultiTemp, sel(0)-1)
              saveChange=2
            ENDIF
          ENDIF
        ENDIF
                ...
                IF szQ EQ 'STRUCT' THEN BEGIN
                  IF exTempNames.HasValue(tempname) THEN BEGIN
                    ...
                    IF sv EQ 'Yes' THEN BEGIN
                      ...
                      quickTemp=replaceStructStruct(quikcTemp, markedMultiTemp, numb)
                      saveChange=2
                    ENDIF ELSE saveChange=0
                  ENDIF ELSE quickTemp=CREATE_STRUCT(quickTemp, tempname, markedMultiTemp)
                ENDIF ELSE quickTemp=CREATE_STRUCT(tempname, markedMultiTemp)
              ENDIF ELSE quickTemp=CREATE_STRUCT(tempname, markedMultiTemp)
    ...
    'manageQT': BEGIN
      ...
      IF N_ELEMENTS(quickTemp) NE 0 THEN BEGIN
        ...
        manageQT, GROUP_LEADER=ev.TOP, xoffset+200, yoffset+200
        RESTORE, thisPath+'data\config.dat'
