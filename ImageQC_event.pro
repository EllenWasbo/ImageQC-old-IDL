...
pro ImageQC_event, ev
  ...
  IF N_ELEMENTS(uval) GT 0 AND SIZE(uval, /TNAME) EQ 'STRING' THEN BEGIN
    ...
    CASE uval OF
      ...
      'config': BEGIN
        ...
        ;PET tests
        WIDGET_CONTROL, txtCrossROIsz, GET_VALUE=txtCrossROIsz
        WIDGET_CONTROL, txtCrossVol, GET_VALUE=txtCrossVol
        ...
        config=CREATE_STRUCT('defPath',defPath,$
          ...
          'CrossROIsz', FLOAT(txtCrossROIsz(0)), 'CrossVol', FLOAT(txtCrossROIsz(0)) )
      ...
      'dim': BEGIN
        IF tags(0) NE 'EMPTY' THEN BEGIN
          ...
          results(getResNmb(WIDGET_INFO(wtabModes, /TAB_CURRENT),'DIM',analyseStringsCT,analyseStringsXray,analyseStringsNM,analyseStringsPET))=1
          dimRes=resArr
          updateTable
          updatePlot, 0,0,0
          redrawImg,0,0
      ...
      'drawROIstp': BEGIN
        IF tags(0) NE 'EMPTY' THEN BEGIN
          ...
          results(getResNmb(WIDGET_INFO(wtabModes, /TAB_CURRENT),'STP',analyseStringsCT,analyseStringsXray,analyseStringsNM,analyseStringsPET))=0
      ...
      'STPpix': BEGIN
        IF tags(0) NE 'EMPTY' THEN BEGIN
          IF analyse NE 'STP' THEN sv=DIALOG_MESSAGE('Show ROIs first to verify size and positions.',/INFORMATION) ELSE BEGIN
            ...
            results(getResNmb(WIDGET_INFO(wtabModes, /TAB_CURRENT),'STP',analyseStringsCT,analyseStringsXray,analyseStringsNM,analyseStringsPET))=1
            updateTable
            updatePlot, 0,1,0
            WIDGET_CONTROL, wtabResult, SET_TAB_CURRENT=0
          ENDELSE
      ...
      'impDose': BEGIN
        IF tags(0) NE 'EMPTY' THEN BEGIN
          IF results(getResNmb(WIDGET_INFO(wtabModes, /TAB_CURRENT),'STP',analyseStringsCT,analyseStringsXray,analyseStringsNM,analyseStringsPET)) THEN BEGIN
            IF nFrames EQ 0 THEN nImg=N_TAGS(structImgs) ELSE nImg=nFrames
            markedArr=INTARR(nImg)
            IF marked(0) EQ -1 THEN markedArr=markedArr+1 ELSE markedArr(marked)=1
      ...
      'drawROIhomog': BEGIN
        IF tags(0) NE 'EMPTY' THEN BEGIN
          ...
          results(getResNmb(WIDGET_INFO(wtabModes, /TAB_CURRENT),analyse,analyseStringsCT,analyseStringsXray,analyseStringsNM,analyseStringsPET))=0
          homogRes=0
          updateTable
          updatePlot, 0,0,0
      ...
      'homog': BEGIN
        IF tags(0) NE 'EMPTY' THEN BEGIN
          IF analyse NE 'HOMOG' THEN sv=DIALOG_MESSAGE('Show ROIs first to verify size and positions.',/INFORMATION) ELSE BEGIN
            ...
            homogRes=resArr
            results(getResNmb(WIDGET_INFO(wtabModes, /TAB_CURRENT),analyse,analyseStringsCT,analyseStringsXray,analyseStringsNM,analyseStringsPET))=1
            updateTable
            updatePlot, 1,1,0
            WIDGET_CONTROL, wtabResult, SET_TAB_CURRENT=0
            curMode=WIDGET_INFO(wtabModes, /TAB_CURRENT)
            IF curMode EQ 3 THEN WIDGET_CONTROL, wtabResult, SET_TAB_CURRENT=1; PET showing slice-to-slice variation as plot rather than table
          ENDELSE
        ENDIF
      END

      'cw_homogNM':  BEGIN
        IF analyse EQ 'HOMOG' AND results(getResNmb(WIDGET_INFO(wtabModes, /TAB_CURRENT),analyse,analyseStringsCT,analyseStringsXray,analyseStringsNM,analyseStringsPET)) EQ 1 THEN BEGIN
          results(getResNmb(WIDGET_INFO(wtabModes, /TAB_CURRENT),'HOMOG',analyseStringsCT,analyseStringsXray,analyseStringsNM,analyseStringsPET))=0
          homogRes=!Null
          analyse='NONE'
          redrawImg,0,0
          updateTable
      ...
      'cw_plotMTF':  BEGIN
        IF analyse EQ 'MTF' AND results(getResNmb(WIDGET_INFO(wtabModes, /TAB_CURRENT),analyse,analyseStringsCT,analyseStringsXray,analyseStringsNM,analyseStringsPET)) EQ 1 THEN BEGIN
          updatePlot, 1,1,0
        ENDIF
      END
      ...
      'Linearity': BEGIN; exctract results
        IF tags(0) NE 'EMPTY' THEN BEGIN
          IF analyse NE 'CTLIN' THEN sv=DIALOG_MESSAGE('Show ROIs first to verify size and positions.',/INFORMATION) ELSE BEGIN
            ...
            CTlinROIpos=INTARR(2,szROI(2),nImg)
            markedArr=INTARR(nImg)
            IF marked(0) EQ -1 THEN markedArr=markedArr+1 ELSE markedArr(marked)=1
            errStatus=0
            errSearch=0
              ...
              IF markedArr(i) THEN BEGIN
                ...
                IF ARRAY_EQUAL(imszTemp[0:1], szROI[0:1]) THEN BEGIN
                  FOR r=0, szROI(2)-1 DO BEGIN
                    ...
                    ;find center
                    IMAGE_STATISTICS, tempImg, MINIMUM=mini, MAXIMUM=maxi, MASK=searchMask
                    halfMax=(mini+maxi)/2
                    ...
                    centerPos=-1
                    IF searchAvoid EQ 0 THEN centerPos=ROUND(centroid(tempImg[xnonZero(0):xnonZero(0)+rad2,ynonZero(0):ynonZero(0)+rad2], halfmax))
                    ...
                    CTlinROIpos[*,r,i]=centerPos                    
                    statMask=getSampleRois(imszTemp, [-imszTemp(0)/2,-imszTemp(1)/2,0,0], rad1, centerPos)                               
                    IMAGE_STATISTICS, tempImg, MEAN=meanHU, MASK=statMask
                    resArr(r,i)=meanHU
                  ENDFOR
            ...
            WIDGET_CONTROL, lblProgress, SET_VALUE=''
            If errstatus THEN sv=DIALOG_MESSAGE('The images have different size. Results restricted to images with same size.',/INFORMATION)
            IF errSearch THEN sv=DIALOG_MESSAGE('Failed searching center for one or more samples. Geometric center of search ROI is used.')
            CTlinRes=resArr
            results(getResNmb(WIDGET_INFO(wtabModes, /TAB_CURRENT),analyse,analyseStringsCT,analyseStringsXray,analyseStringsNM,analyseStringsPET))=1
            updateTable
            updatePlot, 1,1,0
      ...
      'loadSpectrum': BEGIN
        ...
        IF adr NE '' THEN BEGIN
          ...
          ENDIF ELSE BEGIN
            ...
            analyse='NONE'
            energyRes=!Null
            results(getResNmb(WIDGET_INFO(wtabModes, /TAB_CURRENT),'ENERGYSPEC',analyseStringsCT,analyseStringsXray,analyseStringsNM,analyseStringsPET))=0
            updateTable
            updatePlot, 0,0,0
          ENDELSE
      ...
      'radialProfile': BEGIN
        ...
        IF tags(0) NE 'EMPTY' THEN BEGIN
          ...
          If errstatus THEN sv=DIALOG_MESSAGE('The images have different size. Results restricted to images with same size.',/INFORMATION)
          analyse='RADIAL'
          results(getResNmb(WIDGET_INFO(wtabModes, /TAB_CURRENT),'RADIAL',analyseStringsCT,analyseStringsXray,analyseStringsNM,analyseStringsPET))=1
          updateTable
          updatePlot, 1,1,0
          redrawImg, 0,0
          WIDGET_CONTROL, wtabResult, SET_TAB_CURRENT=1
      ...
      'plotScanSpeed':BEGIN
        IF tags(0) NE 'EMPTY' THEN BEGIN
          WIDGET_CONTROL, /HOURGLASS
          analyse='SCANSPEED'
          results(getResNmb(WIDGET_INFO(wtabModes, /TAB_CURRENT),'SCANSPEED',analyseStringsCT,analyseStringsXray,analyseStringsNM,analyseStringsPET))=1
          updateTable
          updatePlot, 1,1,0
          redrawImg, 0,0
          WIDGET_CONTROL, wtabResult, SET_TAB_CURRENT=1
      ...
      'cross': BEGIN
        ;get ROI values
        IF tags(0) NE 'EMPTY' THEN BEGIN
          IF analyse NE 'CROSSCALIB' THEN sv=DIALOG_MESSAGE('Show ROIs first to verify size and positions.',/INFORMATION) ELSE BEGIN
            ...
            corrFactor=FLOAT(actConc(0))/meanOfmean; (or opposite????)
            WIDGET_CONTROL, txtCrossFactor, SET_VALUE=STRING(corrFactor, FORMAT='(f0.4)')
            crossRes=resArr
      ...
      'updateCross': BEGIN
        ...
        IF restTmin LT actTmin THEN errMsg=errMsg+'ERROR: Time of measuring rest-activity should be later than time of measuring activity.'+newline
        ...
        IF scanTmin GT 0 THEN BEGIN
          ...
          WIDGET_CONTROL, txtCrossConc, SET_VALUE=STRING(conc, FORMAT='(f0.'+nDecimals(conc)+')')

          ;calculate calibration factor if
          IF N_ELEMENTS(crossRes) GT 1 THEN WIDGET_CONTROL, txtCrossFactor, SET_VALUE=STRING(conc/crossRes(2,0), FORMAT='(f0.4)') ELSE WIDGET_CONTROL, txtCrossFactor, SET_VALUE='-'
    ...
    IF action EQ 1 THEN BEGIN
      CASE ev.ID OF
        ...
        txtCrossMeasAct: BEGIN
          ...
          val=ABS(FLOAT(comma2pointFloat(val(0))))
          WIDGET_CONTROL, txtCrossMeasAct, SET_VALUE=STRING(val, FORMAT='(f0.'+nDecimals(val)+')')
        ...
        txtCrossMeasRest: BEGIN
          ...
          WIDGET_CONTROL, txtCrossMeasRest, SET_VALUE=STRING(valR, FORMAT='(f0.'+nDecimals(valR)+')')
          WIDGET_CONTROL, txtCrossScanAct, SET_VALUE='-'
          WIDGET_CONTROL, txtCrossConc, SET_VALUE='-'
          WIDGET_CONTROL, txtCrossFactor, SET_VALUE='-'
        END
        ...
        txtCrossVol: BEGIN
          ...
          WIDGET_CONTROL, txtCrossVol, SET_VALUE=STRING(valV, FORMAT='(f0.'+nDecimals(valV)+')')
          WIDGET_CONTROL, txtCrossConc, SET_VALUE='-'
          WIDGET_CONTROL, txtCrossFactor, SET_VALUE='-'
        END
  ...
  IF TAG_NAMES(ev, /STRUCTURE_NAME) EQ 'WIDGET_TABLE_CELL_SEL' THEN BEGIN
    ...
    CASE curMode OF
      ...
      1:curTab=WIDGET_INFO(wtabAnalysisXray, /TAB_CURRENT)
      2:curTab=WIDGET_INFO(wtabAnalysisNM, /TAB_CURRENT)
      2:curTab=WIDGET_INFO(wtabAnalysisPET, /TAB_CURRENT)
    ENDCASE
  ...
  IF (TAG_NAMES(ev, /STRUCTURE_NAME) EQ 'WIDGET_TAB') THEN BEGIN
    ...
    CASE ev.ID OF
      ...
      wtabAnalysisPET: BEGIN
        IF loadedImg THEN BEGIN
          IF results(selTab) EQ 1 THEN analyse=analyseStringsPET(selTab+1) ELSE analyse=analyseStringsPET(0)
        ENDIF
      END
      ELSE:
    ENDCASE
