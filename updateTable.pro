      ...
      IF results(curTab) EQ 1 THEN BEGIN
        ...
        CASE analyseStringsCT(curTab+1) OF
          ...
          'HOMOG': BEGIN
            ...
            nCols=szTab(0)
            headers=['Center HU','12oClock','15oClock','18oClock','21oClock','Std center', 'Std 12','Std 15','Std 18','Std 21']
            resArrString=STRARR(nCols,nRows)
            FOR i=0, nRows-1 DO BEGIN
      ...
      IF results(curTab) EQ 1 THEN BEGIN
        ...
        CASE analyseStringsPET(curTab+1) OF
          'CROSSCALIB': BEGIN
            ...
            resArrString=STRARR(nCols,nRows)
            FOR i=0, nRows-1 DO resArrString[*,i]=STRING(crossRes[*,markedTemp(i)],FORMAT='(f0.'+nDecimals(crossRes[*,markedTemp(i)])+')')
          ...
          'HOMOG': BEGIN
            ...
            nDecVal=nDecimals(MEAN(resMarked[0:4,*]))
            nDecDiff=nDecimals(MEAN(resMarked[5:9,*]))
            FOR i=0, nRows-1 DO BEGIN
              resMarked[5:9,i]=100.*(resMarked[0:4,i]-meanVals)/meanVals
              resArrString[0:4,i]=STRING(resMarked[0:4,i], FORMAT='(f0.'+nDecVal+')')
              resArrString[5:9,i]=STRING(resMarked[5:9,i], FORMAT='(f0.'+nDecDiff+', "%")')
            ENDFOR  
