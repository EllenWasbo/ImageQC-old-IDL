...
function getSeriesTable, struct
  ...
  IF SIZE(struct, /TNAME) EQ 'STRUCT' THEN BEGIN
    nSer=N_TAGS(struct)
    serTab=STRARR(6,nSer)
    FOR i=0, nSer-1 DO serTab[*,i]=[struct.(i).(0).seriesName,struct.(i).(0).kVp,struct.(i).(0).mAs,struct.(i).(0).kernel,struct.(i).(0).acqDate+'/'+STRMID(struct.(i).(0).acqtime,0,6),STRING(N_TAGS(struct.(i)),FORMAT='(i0)')]
  ENDIF
  return, serTab
end
...
pro autoMTFNPS, strucParams
  ...
  MTFNPSbox = WIDGET_BASE(TITLE='Automatic analyze of task based MTF and NPS', /COLUMN, XSIZE=1200, YSIZE=820, XOFFSET=20, YOFFSET=20)
  ...
  lblLocalTip = WIDGET_LABEL(toolbar, VALUE='NB - local files quicker accessed than network files', FONT=font1)
  ;toolInfo=WIDGET_BUTTON(toolbar, VALUE='i', UVALUE='infoMTFNPS', TOOLTIP='Information about this program', fONT=font0)
  ...
  tabMTF=WIDGET_TABLE(btabMTF, XSIZE=6, COLUMN_LABELS=['Series name','kV','mAs','kernel','acqDate/Time','nImg'],ALIGNMENT=1,COLUMN_WIDTHS=[150,50,50,70,110,50],/NO_ROW_HEADERS, SCR_XSIZE=510, SCR_YSIZE=190, /ALL_EVENTS, FONT=font1)
  ...
  ml000=WIDGET_LABEL(bButtonsMTF, VALUE='', YSIZE=15)
  btnCalcMTF=WIDGET_BUTTON(bButtonsMTF, VALUE='Calculate MTF', UVALUE='calcMTF',FONT=font1)
  ml001=WIDGET_LABEL(bButtonsMTF, VALUE='', YSIZE=15)
  ...
  mlNPSprevnext=WIDGET_LABEL(bPrevNextNPS, VALUE='', XSIZE=10)
  ...
  tabNPS=WIDGET_TABLE(btabNPS, XSIZE=6, COLUMN_LABELS=['Series name','kV','mAs','kernel','acqDate/Time','nImg'],ALIGNMENT=1,COLUMN_WIDTHS=[150,50,50,70,110,50],/NO_ROW_HEADERS, SCR_XSIZE=510, SCR_YSIZE=190,/ALL_EVENTS, FONT=font1)
  ...
  ml000=WIDGET_LABEL(bButtonsNPS, VALUE='', YSIZE=15)
  ...
  lblRangeNPSX = WIDGET_LABEL(bRangeNPSX, VALUE='Horizontal axis range (lower, upper)', XSIZE=200,FONT=font1)
  txtMinRangeNPS_X = WIDGET_TEXT(bRangeNPSX, VALUE='0', /EDITABLE, XSIZE=7, SCR_YSIZE=20, /KBRD_FOCUS_EVENTS, FONT=font1)
  lblMlmRNPSx= WIDGET_LABEL(bRangeNPSX, VALUE=', ', XSIZE=10, FONT=font1)
  ...
  lblRangeNPSY = WIDGET_LABEL(bRangeNPSY, VALUE='Vertical axis range (lower, upper)', XSIZE=200,FONT=font1)
  txtMinRangeNPS_Y = WIDGET_TEXT(bRangeNPSY, VALUE='0', /EDITABLE, XSIZE=7, SCR_YSIZE=20, /KBRD_FOCUS_EVENTS, FONT=font1)
  lblMlmRNPSy= WIDGET_LABEL(bRangeNPSY, VALUE=', ', XSIZE=10, FONT=font1)
...
pro redrawSeries
  ...
  IF N_ELEMENTS(structImgsCurr) NE 0 THEN BEGIN
    ...
    imgNos=INTARR(3)
    FOR i=0,2 DO imgNos(i)=structImgsCurr.(serNo).(firstImgNoCurr+i).imgNo
...
pro autoMTFNPS_event, event
  ...
  IF N_ELEMENTS(uval) GT 0 THEN BEGIN
    CASE uval OF
      'openMTFNPS':BEGIN
        ;search all images
        adr=DIALOG_PICKFILE(TITLE='Select the folder', DIALOG_PARENT=evTop, PATH=defPath, GET_PATH=thisPath, /DIRECTORY)
        ...
        IF adr NE '' THEN BEGIN
          ...
          IF nFiles GT 0 THEN BEGIN
            ...
            IF nMTF GT 3 THEN WIDGET_CONTROL, btnNextMTF, SENSITIVE=1 ELSE WIDGET_CONTROL, btnNextMTF, SENSITIVE=0
            IF nNPS GT 3 THEN WIDGET_CONTROL, btnNextNPS, SENSITIVE=1 ELSE WIDGET_CONTROL, btnNextNPS, SENSITIVE=0
      ...
      'remThisMTF':BEGIN
        IF N_ELEMENTS(structImgsMTF) GT 0 THEN BEGIN
          ...
          IF nImgSer GT 1 THEN BEGIN
            ...
            acqdateTime=TRANSPOSE(currTab[4,*])
            ...
            IF sameAcq(0) NE -1 THEN BEGIN
              FOR i=0, nSer-1 DO BEGIN
                IF i NE serNo THEN BEGIN
                  FOR n=0, LONG(currTab(5,i))-1 DO IF structImgsMTF.(i).(n).zpos EQ zpos THEN sameZ(i)=n
                ENDIF
              ENDFOR
            ENDIF
          ...
          IF N_ELEMENTS(tMTF) NE 1 THEN BEGIN
            ...
            ENDIF ELSE BEGIN
              ...
              nImgSer=nImgSer-1
            ENDELSE
          ENDIF ELSE nImgSer=LONG(tMTF(5))
        ...
        IF N_ELEMENTS(structImgsNPS) GT 0 THEN BEGIN
          ...
          IF nImgSer GT 1 THEN BEGIN
            ...
            IF sameAcq(0) NE -1 THEN BEGIN
              FOR i=0, nSer-1 DO BEGIN
                IF i NE serNo THEN BEGIN
                  FOR n=0, LONG(currTab(5,i))-1 DO IF structImgsNPS.(i).(n).zpos EQ zpos THEN sameZ(i)=n
                ENDIF
              ENDFOR
            ENDIF
            delSameID=WHERE(sameZ GT -1)
          ...
          IF N_ELEMENTS(tNPS) NE 1 THEN BEGIN
            ...
            ENDIF ELSE BEGIN
              ...
              nImgSer=nImgSer-1
            ENDELSE
          ENDIF ELSE nImgSer=LONG(tNPS(5))
      ...
      'remSeriesMTF':BEGIN
        ...
        IF nMTF GT 1 THEN nImgSer=LONG(tMTF(5,0)) ELSE nImgSer=LONG(tMTF(5))
        WIDGET_CONTROL, tabMTF, ALIGNMENT=1, TABLE_YSIZE=nMTF, SET_VALUE=tMTF
        WIDGET_CONTROL, tabMTF, SET_TABLE_SELECT=[0,0,0,0]
      ...
      'remSeriesNPS':BEGIN
        ...
        IF nNPS GT 1 THEN nImgSer=LONG(tNPS(5,0)) ELSE nImgSer=LONG(tNPS(5))
        WIDGET_CONTROL, tabNPS, ALIGNMENT=1, TABLE_YSIZE=nNPS, SET_VALUE=tNPS
        WIDGET_CONTROL, tabNPS, SET_TABLE_SELECT=[0,0,0,0]
      ...
      'exportMTFNPS':BEGIN
        ...
        IF adr NE '' THEN BEGIN
          ...
          includeTagsShort=STRUPCASE(['seriesName','kVp','mAs','kernel','acqDate/time','nImgs'])
          includeTagsExt=STRUPCASE(['institution','stationName','modelName','patientName','patientID','imageType','studyDescr','protocolname','coll','sliceThick','mA','ExpTime','pitch','rekonFOV','pix','CTDIvol'])
          ...
          FOR o=0, 2 DO BEGIN
              ...
              1:BEGIN;NPS
                IF N_ELEMENTS(NPSres) GT 0 THEN BEGIN
                  ...
                  FOR s=0, nSer-1 DO BEGIN
                    ...
                    headersPlot=CREATE_STRUCT(headersPlot, 'C'+STRING(s,FORMAT='(i0)'),[structImgsNPS.(s).(0).seriesName, structImgsNPS.(s).(0).kernel, structImgsNPS.(s).(0).mAs,'','NPS'])
                  ENDFOR
                ENDIF
              END
            ...
            IF nnPlots GT 1 THEN BEGIN
              ...
              headArr=STRARR(nnPlots+1,5)
              topInfo=TRANSPOSE(['SeriesName','Kernel','mAs','Material','Frequency mm-1'])
              IF o EQ 1 THEN topInfo(3)=''
              headArr[0,0:4]=topInfo
              ...
              FOR i=0, nnPlots-1 DO BEGIN
                    ...
                    headArr[i+1,0:4]=TRANSPOSE(headersPlot.(i))
              ENDFOR
  ...
  IF TAG_NAMES(event, /STRUCTURE_NAME) EQ 'WIDGET_TABLE_CELL_SEL' THEN BEGIN
    IF event.ID EQ tabMTF THEN BEGIN
      IF event.sel_top NE -1 THEN BEGIN
        ...
        WIDGET_CONTROL, btnPrevMTF, SENSITIVE=0
        WIDGET_CONTROL, tabMTF, GET_VALUE=currTab
        nImgSer=LONG(currTab(5,event.sel_top))
...
pro clearMTFNPS
  ...
  WIDGET_CONTROL, resTabMTF, TABLE_YSIZE=1, SET_VALUE=STRARR(6)
  WIDGET_CONTROL, resTabNPS, TABLE_YSIZE=1, SET_VALUE=STRARR(6)
end